 ...
 *
 * This variant is for kernels where /dev/mtdN character devices return ENXIO
 * but /dev/mtdblockN is available.
 *
 * SAFETY:
 *   - Can write ONLY /dev/mtdblock2 or /dev/mtdblock3.
 *   - Refuses unless /proc/bootbank reports Bank 1 active.
...
static int active_bank1(void){
    ...
    if(v[0]!='1'){
        out("REFUSING: this flasher is only allowed while Bank 1 is active.\n");
        return 0;
    }
    return 1;
}
...
static int app(void){
    static char sr[]="/tmp/FLASH_BANK2_ROOTFS_NOW";
    static char sk[]="/tmp/FLASH_BANK2_KERNEL_NOW";
    static char fr[]="/tmp/mtd3-bank2-rootfs-reclaimed.bin";
    static char fk[]="/tmp/mtd2-bank2-kernel-reclaimed.bin";
    static char dr[]="/dev/mtdblock3";
    static char dk[]="/dev/mtdblock2";
    int rmode,kmode;

    out("bank2-safe-flash-v2-block\n");
    out("Writes ONLY inactive Bank 2 through the RTL mtdblock driver.\n");

    if(!active_bank1())return 2;
    ...
    if(rmode==kmode){
        ...
        out("  /tmp/FLASH_BANK2_ROOTFS_NOW\n");
        out("  /tmp/FLASH_BANK2_KERNEL_NOW\n");
        return 3;
    }

    if(rmode)return flash_one(fr,dr,ROOTFS_SIZE,"BANK2 ROOTFS / mtdblock3");
    return flash_one(fk,dk,KERNEL_SIZE,"BANK2 KERNEL / mtdblock2");
}
